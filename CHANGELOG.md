...
- Enh: Custom side menu via `common\modules\rbac\Module::items` (mdmunir).
...
- Chg: Can customize return of `common\modules\rbac\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `common\modules\rbac\components\BizRule` (mdmunir).
...
- Chg: `db` and `cache` configuration via `$app->params['common.modules.rbac.configs']` (mdmunir).
- Chg: #29: Change `yii\caching\GroupDependency` to `yii\caching\TagDependency` (mdmunir).
- Chg: Remove `common\modules\rbac\Module::allowActions`. Set access control directly with `common\modules\rbac\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `common\modules\rbac\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
